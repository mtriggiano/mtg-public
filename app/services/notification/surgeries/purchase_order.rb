class Notification::Surgeries::PurchaseOrder < NotificationMaker
  ...
  def for_created
    ...
    super(title, body)
    Activity::Surgeries::PurchaseOrder.new(document.supplier.set_activity(current_user), document).for_new
  end
  ...
  def for_disable
    title = "Se anuló una orden de compra a proveedor"
    body = "La orden de compra Nº #{document.number} fue anulada"
    super(title, body)
    Activity::Surgeries::PurchaseOrder.new(document.supplier.set_activity(current_user), document).for_new
  end
