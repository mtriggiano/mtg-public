module SupplierManager
  class MassivePaymentOrderImporter
    attr_accessor :file, :supplier_id, :date, :current_user

    EXTENSIONES_PERMITIDAS = %w(.xls .xlsx)

    FILAS_ENCABEZADO = 1

    def initialize(file, supplier_id, date, current_user)
      @file = file
    ...
    def registra_transacciones(vector_de_filas)
      ...
      payment_order = Purchases::PaymentOrder.new(
        company_id: 3,
      ...
      vector_de_filas.each_with_index do |fila, index|
        ...
        bill = ExternalBill.approveds.where(company_id: 3, entity_id: @supplier_id).where("CAST(bills.sale_point as int) = :sale_point AND CAST(bills.number as int) = :number", sale_point: registro[:punto_venta].to_i, number: registro[:number].to_i).first
        unless bill.nil?
          payment_order.details.where(
            bill_id: bill.id,
            total: registro[:total].to_f
          ).first_or_create
          total_payment_order += registro[:total].to_f
        end
      end
      payment_order.update_columns(total: total_payment_order)
