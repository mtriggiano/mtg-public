class Reports::ShipmentDatatable < ApplicationDatatable

	def data
	  	records.map do |record|
	  		presenter = Reports::StoragePresenter.new(record, @view)
	  		result = Hash.new
	    	DatatableConstants::Reports::Shipping.keys.map do |col|
	    		result[col.to_sym] = eval("presenter.#{col}")
	    	end
	    	result[:actions] = presenter.action_links
				result[:DT_RowId] = record.id
	    	result
	  	end
	end

	def view_columns
		@view_columns = Hash.new
		DatatableConstants::Reports::Shipping.map do |k, v|
			@view_columns[k.to_sym] = v.except(:name)
		end
		@view_columns[:links] = {}
		return @view_columns
	end

	def get_raw_records
		@collection
			.joins(:file)
			.joins(:client)
			.joins("LEFT JOIN orders_shipments ON orders_shipments.shipment_id = shipments.id")
			.joins("LEFT JOIN orders ON orders.id = orders_shipments.order_id")
			.joins("LEFT JOIN order_details ON order_details.order_id = orders.id")
			.joins("LEFT JOIN users on users.id = order_details.user_id")
			.where(state: ["Aprobado", "Confirmado", "Anulado"])
			.distinct("shipments.id")
			.order("shipments.created_at ASC")
	end
end
