...
ext = '.xlsx'
file_paths = [
  base + 'DEPOSITO 4 - JERINGAS CON ERROR' + ext,
]
...
file_paths.each do |file_path|
  ...
  data << {
    ...
    "stock": row[8].to_s.to_i,
    "lote": row[11].to_s,
    "serie": row[12].to_s,
    "vencimiento": row[13].try(:value) || row[12].to_s,
    "vigente": row[14].try(:value) || row[13].to_s,
    "deposito": row[15].to_s,
  }
  end
end

# Después de tu loop que llena la variable 'data'
File.open(base + 'data_nueva.json', 'w') do |file|
