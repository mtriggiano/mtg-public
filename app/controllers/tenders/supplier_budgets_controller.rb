class Tenders::SupplierBudgetsController < ApplicationController
	include Attachable
	include Fileable
	...
	# end
  	...
  	private
  		...
  		def budget_params
  			params.require(:tenders_supplier_budget).permit(:number, :payment_type, :seller_id, :entity_id, :entity_contact_id, :delivery_date, :delivery_address, :init_date, :final_date, :discount, :total, :state, :observation, :file_id, :total_usd, :usd_price, :currency,
      			...
      			custom_attributes: current_company.tender_supplier_budget_configs.pluck(:extra_attribute).map{|attribute| attribute.parameterize.underscore.to_sym}
  			)
  		end
end
