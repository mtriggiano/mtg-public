class Activity::Surgeries::PurchaseOrder < ActivityRecord
  def for_new
    ...
    when "Aprobado"
      ...
      activity.body 		= "El usuario #{activity.user.name} aprobó la orden de compra nº #{document.number} a nombre del cliente el dia #{I18n.l(Date.today, format: :short)} a #{Time.now.strftime("%H:%M")} hs."
    when "Rechazado"
      activity.title 		= "Orden de compra rechazada"
      activity.body 		= "El usuario #{activity.user.name} rechazo la orden de compra nº #{document.number} a nombre del cliente el dia #{I18n.l(Date.today, format: :short)} a #{Time.now.strftime("%H:%M")} hs."
    end
