class AgreementSurgeries::ShipmentsController < ApplicationController
  include Attachable
  include Fileable
  ...
  include Traceable
  expose :shipment, scope: -> {shipments.includes(details: [:product, :batches, childs: [:product, :batches] ])}, model: "AgreementSurgeries::Shipment"
  ...
  def get_details
    shipments = current_company.agerement_surgery_shipments.where(id: params[:shipment_ids])
  ...
  def shipment_params
    params.require(:agreement_surgeries_shipment).permit(:store_id, :file_id, :number, :entity_id, :date, :punctuation, :state, :observation, :shipment_type,
      :from, :to, :id_evento, :conformed, :conformed_file,
       ...
       custom_attributes: current_company.agreement_surgery_shipment_configs.pluck(:extra_attribute).map { |attribute| attribute.parameterize.underscore.to_sym },
       shipments_sale_orders_attributes: %i[id order_id _destroy])
  end
  
  def conformed_params
    params.require(:agreement_surgeries_shipment).permit(:conformed, :conformed_file)
  end
end
